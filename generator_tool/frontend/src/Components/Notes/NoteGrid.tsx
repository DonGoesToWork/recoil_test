...
const truncated = (content: string): string =>
  content.length > line_length_max
    ? content.substring(0, line_length_max).split('\n').join(', ') + '...'
    : content;
      ...
      {notes.map((note) => (
        ...
        >
          ...
          <div className="note-preview">
            {truncated('- ' + note.child_list)}
          </div>
          <div className="note-preview">
            {truncated('- ' + note.property_list)}
          </div>
