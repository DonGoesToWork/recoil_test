...
export default class BaseGenerator {
  ...
  update_lists() {
    this.user_property_list_raw = this.note.user_property_list
      .split('\n')
      .filter((x) => x !== '');

    this.base_property_list = this.note.property_list
      .split('\n')
      .filter((x) => x !== '');
    ...
    this.user_property_list = this.user_property_list_raw.map(
      (x) => x + '_user_input'
    );

    this.user_propery_list_dual = [
      ...this.user_property_list_raw,
      ...this.user_property_list,
    ];

    this.user_base_property_list = [
      ...this.user_property_list,
      ...this.base_property_list,
    ];

    this.combined_property_list = [
      ...this.user_property_list_raw,
      ...this.user_base_property_list,
      'id',
